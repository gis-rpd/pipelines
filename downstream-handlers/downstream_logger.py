...
def dry_run_skip():
    ...
    """
    logger.warning("Dryrun: Skipping Mongo update")
    sys.exit(0)
        ...
        if results.count() == 1:
            ...
            if results.count() == 1:
                ...
                try:
                    if args.dry_run:
                        dry_run_skip()
                    res = db.update({"_id": ObjectId(args.db_id)},
                    ...
                    logger.fatal("MongoDB update failure while updating db-id %s",
                                 args.db_id)
                    sys.exit(1)
                ...
                if "100%" in last_line:
                    try:
                        if args.dry_run:
                            dry_run_skip()
                        db.update({"_id": ObjectId(args.db_id)},
                                  {"$set": {"run.status": "SUCCESS", "run.end_time": end_time}})
                    except pymongo.errors.OperationFailure:
                        logger.fatal("mongoDB OperationFailure")
                        sys.exit(1)
                    ...
                    try:
                        if args.dry_run:
                            dry_run_skip()
                        db.update({"_id": ObjectId(args.db_id)},
                                  {"$set": {"run.status": "FAILED", "run.end_time": end_time}})
